...
### B+树和B树的区别
1. B+树非叶子节点不存储数据，可以存更多的索引值，让树的高度更低，减少扫描的page的数量
2. B+树叶子结点形成链表，更方便做range
3. B+树非叶子节点不存储数据，所以所有的查询最后都要走到叶子节点，总体来说查询耗时更稳定。B树中间节点也包含数据，可能有的查询只查一个page就返回了，有的查询要一直到叶子节点才返回，不公平不均衡不稳定。
